...
exports.createGroupsCommand = createGroupsCommand;
exports.createChatsCommand = createChatsCommand;
...
function formatGroup(group, highlightMatches) {
    console.log(`\n  ${group.name}`);
...
function searchGroups(groups, query) {
    const results = (0, search_1.fuzzySearch)(groups, query, (group) => [group.name, group.description || ''], { threshold: 0.2 });
 ...
 */
async function selectGroup(results) {
    if (results.length === 0) {
        return null;
    }
    if (results.length === 1) {
...
function createGroupsCommand() {
    const cmd = new commander_1.Command('groups')
        ...
        try {
            ...
            if (options.id) {
                const { getGroup } = await Promise.resolve().then(() => __importStar(require('../api')));
            ...
            const groups = await (0, api_1.getGroups)(token);
            ...
            if (options.search) {
                ...
                if (options.exact) {
                    // Require exact match
                    const exactMatch = groups.find(g => g.name.toLowerCase() === options.search.toLowerCase());
                    if (!exactMatch) {
...
function createChatsCommand() {
    const cmd = new commander_1.Command('chats')
        ...
        try {
            ...
            const chats = await (0, api_1.getChats)(token, page, perPage);
            let displayChats = chats;
            // Apply search if specified
            if (options.search) {
                const results = (0, search_1.fuzzySearch)(chats, options.search, (chat) => chat.other_user.name, { threshold: 0.3 });
                if (results.length === 0) {
 ...
 */
async function resolveGroup(token, identifier) {
    ...
    if (results.length === 0) {
        throw new api_1.GroupMeAPIError(`No group found matching "${identifier}"`, null, 'resolveGroup', 404);
    }
