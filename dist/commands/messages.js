...
exports.createDmReadCommand = createDmReadCommand;
exports.createDmSendCommand = createDmSendCommand;
exports.createReplyCommand = createReplyCommand;
...
function formatDirectMessage(msg) {
    const date = formatDate(msg.created_at);
    const likes = msg.favorited_by.length > 0 ? ` [${msg.favorited_by.length} likes]` : '';
    console.log(`\n[${date}] ${msg.name}${likes}`);
    if (msg.text) {
 ...
 */
function formatCompactMessage(msg) {
...
function createReadCommand() {
    const cmd = new commander_1.Command('read')
        ...
        try {
            ...
            try {
                ...
                if (/^\d+$/.test(options.group) || /^[a-f0-9-]{36}$/i.test(options.group)) {
                    ...
                    const { group, exact } = await (0, groups_1.resolveGroup)(token, options.group);
                    groupId = group.id;
            ...
            const opts = {};
            if (options.before)
                opts.before_id = options.before;
            if (options.after)
                opts.after_id = options.after;
            if (limit)
                opts.limit = limit;
            const response = await (0, api_1.getMessages)(token, groupId, opts);
            if (options.json) {
                const output = {
...
function createSendCommand() {
    const cmd = new commander_1.Command('send')
        ...
        try {
            ...
            const message = await (0, api_1.sendMessage)(token, groupId, options.message);
            if (options.json) {
...
function createDmReadCommand() {
    const cmd = new commander_1.Command('dm-read')
        ...
        try {
            ...
            if (options.after)
                opts.after_id = options.after;
            const response = await (0, api_1.getDirectMessages)(token, options.user, opts);
            if (options.json) {
...
function createDmSendCommand() {
    const cmd = new commander_1.Command('dm')
        ...
        try {
            const token = (0, config_1.getToken)(options.token);
            const message = await (0, api_1.sendDirectMessage)(token, options.user, options.message);
            if (options.json) {
                console.log(JSON.stringify(message, null, 2));
                return;
            }
...
function createReplyCommand() {
    const cmd = new commander_1.Command('reply')
        ...
        try {
            ...
            const message = await (0, api_1.sendReply)(token, groupId, options.message, options.replyTo);
            if (options.json) {
                console.log(JSON.stringify(message, null, 2));
                return;
            }
